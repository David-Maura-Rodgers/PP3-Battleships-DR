...
# --- CREATE RANDOM COM SHIP COORDINATES (AND BOARD HOPEFULLY) ------ //     
...
# --- CHECK PLAYER MISSLE AGAINST COM SHIP POSITIONS --- \\
def check_player_missile(player_missile_x, player_missile_y):
    ...
    while should_continue:
        player_missile_x = int(input("Select X coordinates to fire your missle at the ENEMY: "))
        player_missile_y = int(input("Select Y coordinates to fire your missle at the ENEMY: "))
        print("\n")
...
def check_com_missile(enemy_missile_x, enemy_missile_y):
    ...
    while should_continue:
        enemy_missile_x = random.sample(range(1, 6), 1)
        enemy_missile_y = random.sample(range(1, 6), 1)
        print(f"Enemy Missile Strike: {enemy_missile_x}")
        print(f"Enemy Missile Strike: {enemy_missile_y}")
...
def play_game():
    ...
    while not ship_placement:
        ...
        # Updates board with the user input variables and places @ there.
        horizontal = input_x
        vertical = input_y
